...
if __name__ == "__main__":
    ...
    def preprocessing():
        ...
        data["v_dummy"] = np.nan
        data.loc[data["v_t"] == 1, "v_dummy"] = 1
        data.loc[data["v_t"] == 0, "v_dummy"] = -1
        ...
        # Angular shield size from degrees to radians
        data["angular_shield_size"] = np.deg2rad(data["angular_shield_size"])
        ...
        for i in range(n_subj):
            ...
            if not np.sum(np.isnan(df_subj["v_dummy"])) == 0:
                sys.exit("Unexpected NaNs in v_dummy")
            ...
            if not np.sum(np.isnan(df_subj["group"])) == 0:
                sys.exit("Unexpected NaNs in group")
            if not np.sum(np.isnan(df_subj["angular_shield_size"])) == 0:
                sys.exit("Unexpected NaNs in angular_shield_size")
