"""SCA analysis for fixed learning rate."""
...
if __name__ == "__main__":
    ...
    for i, (name, spec) in enumerate(regression_specs.items()):
        result = run_or_load_model(
            name, spec, reg_vars, df_for, force_rerun=force_rerun
        )
        # Store results for model comparison
        BIC.append(sum(result["BIC"]))
        fixed_LR.append(np.mean(result["beta_1"]))
        all_LRs[:, i] = result["beta_1"]
    ...
    # ------------------------------------
    # Run checks whether we have all files
    # ------------------------------------
    ...
    all_factors, all_betas, all_results = run_sca(
        regression_specs, analysis_specs, reg_vars, df_for, which_factor
    )
