...
if __name__ == "__main__":
    ...
    from rbm_analyses import AgentVars
    ...
    if est_vars.which_vars["omikron_0"]:
        if use_subject_estimates:
            ...
            df_model.loc[:, "omikron_0"] = np.random.uniform(low=5.0, high=10.0, size=n_subj)
    else:
        df_model.loc[:, "omikron_0"] = np.repeat(est_vars.fixed_mod_coeffs['omikron_0'], n_subj)

    if est_vars.which_vars["omikron_1"]:
        if use_subject_estimates:
            df_model.loc[:, "omikron_1"] = df_estimates["omikron_1"].to_numpy()
        else:
            df_model.loc[:, "omikron_1"] = np.random.uniform(low=0.0, high=0.3, size=n_subj)
    else:
        df_model.loc[:, "omikron_1"] = np.repeat(est_vars.fixed_mod_coeffs['omikron_1'], n_subj)
    ...
    if est_vars.which_vars["sigma_H"]:
        if use_subject_estimates:
            ...
            df_model.loc[:, "sigma_H"] = np.random.uniform(low=0.0, high=0.5, size=n_subj)
    else:
        df_model.loc[:, "sigma_H"] = np.repeat(est_vars.fixed_mod_coeffs['sigma_H'], n_subj)
