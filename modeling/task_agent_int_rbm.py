...
def task_agent_int(
    ...
    kappa = np.full(n_trials, np.nan)  # response noise
    ...
    for t in range(0, n_trials - 1):
        ...
        if not df["new_block"][t + 1]:
            ...
            p_a_t = vonmises.pdf(df["a_t_rad"][t], loc=a_hat[t], kappa=kappa[t])
            ...
            if sim:
                ...
                sim_a_t[t] = np.random.vonmises(a_hat[t], kappa[t])
                p_a_t_sim = vonmises.pdf(sim_a_t[t], loc=a_hat[t], kappa=kappa[t])
