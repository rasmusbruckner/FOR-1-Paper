"""Preprocess questionnaire data to obtain sum scores."""
...
# Todo: add justification why we exclude them
exclude_ids = [20031, 20098, 99999]
...
# Drop each sub with any nan value -> resulting list of subs has to match hashim's list
qns_data.dropna(axis=0, inplace=True)

# Create a mapping dictionary for renaming columns
column_mapping = {}

# Note: Currently not necessary because we dropped these items
...
# Initialize data frame for sum scores
...
df_totalscore = df_totalscore[~df_totalscore["subj_num"].isin(exclude_ids)]
df_totalscore = df_totalscore.sort_values(by=["subj_num"])

# Adjust index
df_totalscore = df_totalscore.reset_index(drop=True)

# Recode gender for regression dummy style
df_totalscore["gender"] = df_totalscore["gender"].replace({1: -1, 2: 1})

# Save data frame
df_totalscore.name = "questionnaire_sumscores"
